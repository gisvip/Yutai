...
{
    public partial class GeometryInfoView : DockPanelControlBase, IGeometryInfoView
    {
        ...
        public GeometryInfoView(IAppContext context)
        {
            InitializeComponent();
            _context = context;
            this.m_pSymbol = new SimpleMarkerSymbol() as ISymbol;
        ...
        public const string DefaultDockName = "Editor_Feature_GeometryInfo";
        ...
        private void CreateMultiPoint(string filename)
        {
            try
            {
                using (StreamReader reader = new StreamReader(filename))
                {
                    ...
                    {
                        string str2;
                        IPoint inPoint = new ESRI.ArcGIS.Geometry.Point();
                        num++;
        ...
        private void CreatePoint(string filename)
        {
            try
            {
                ...
                {
                    string str;
                    IPoint pGeom = new ESRI.ArcGIS.Geometry.Point();
                    int num2 = 0;
                    while ((str = reader.ReadLine()) != null)
                    {
                        string str2;
            ...
            try
            {
                using (StreamReader reader = new StreamReader(filename))
                {
                    ...
                    if (line != null)
                    {
                        IPoint point;
                        string str2;
                        IGeometryCollection geometrys = new Polygon() as IGeometryCollection;
                        IPointCollection points = new Ring();
                        object before = Missing.Value;
                        bool flag = false;
            ...
            try
            {
                using (StreamReader reader = new StreamReader(filename))
                {
                    ...
                    if (line != null)
                    {
                        ...
                        IPointCollection points = new ESRI.ArcGIS.Geometry.Path();
                        object before = Missing.Value;
                        bool flag = false;
                        if (line.ToLower() == "part")
                        {
                        ...
                        while ((line = reader.ReadLine()) != null)
                        {
                            ...
                            if (line.Length != 0)
                            {
                                ...
                                if (line.ToLower() == "part")
                                {
                                    ...
                                    {
                                        geometrys.AddGeometry(points as IGeometry, ref before, ref before);
                                        points = new ESRI.ArcGIS.Geometry.Path();
                                    }
                                    else
                                    {
                                        str2 = filename + " 中第 " + nIndex.ToString() + "行数据有错误";
        ...
        private void MPActiveViewEventsOnAfterDraw(IDisplay display, esriViewDrawPhase phase)
        {
        ...
        private void MPActiveViewEventsOnSelectionChanged()
        {
